...
schema = StructType([
    ...
    StructField("lostreason_id", IntegerType(), False),
    StructField("chrt_id", LongType(), False)
])
...
columns_to_ch = ("shk_id", "lostreason_id", "chrt_id")
...
sql_insert = f"""insert into {ch_db_name}.{ch_dst_table}
    ...
    (
        select chrt_id
             , subject_id
        from default.dict_Chrt
    ) t2
    on t1.chrt_id = t2.chrt_id
"""
